...
PLUGIN_SKILLS_DIR="$REPO_ROOT/assets/opencode/skills"
...
for skill in "${SKILLS[@]}"; do
  cp -R "$SOURCE_DIR/$skill" "$PLUGIN_SKILLS_DIR/$skill"
  file_count="$(find "$PLUGIN_SKILLS_DIR/$skill" -type f | wc -l | tr -d ' ')"
  echo "  $skill ($file_count files)"
done

echo ""
echo "Synced ${#SKILLS[@]} skills into assets/opencode/skills/"
